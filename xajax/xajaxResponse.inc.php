 ...
 * 
 * @package xajax
 * @version $Id: xajaxResponse.inc.php,v 1.3 2006/05/24 14:55:25 mstar Exp $
			...
			foreach($queryParts as $key => $value)
			{
				if(is_array($value)){
					foreach ($value as $key2 => $value2) {
						$newQueryPart .= rawurlencode($key).'['.rawurlencode($key2).']'.'='.rawurlencode($value2).ini_get('arg_separator.output');
					}
				}else
