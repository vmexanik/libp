 ...
 *
 * @package xajax
 * @version $Id: xajax.inc.php,v 1.3 2006/05/24 14:55:25 mstar Exp $
...
class xajax
{
	...
	function fixParseStrBug(&$aArray) {
		...
		if (is_array($aArray)) {
			foreach ($aArray as $sKey => $sValue) {
				$aArray[$sKey] = $this->fixParseStrBug($sValue);
			}
		} else {
	...
	{
		//-added by mstar-
		if ($sData && is_array($sData)) foreach ($sData as $key =>$value) {
